...
export default function RegisterPage() {
  ...
  const { connectWallet, account, isConnected, checkIfWalletIsInstalled } =
    useWeb3();
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (!checkIfWalletIsInstalled()) {
        showError(
          "MetaMask หรือ wallet อื่นๆ ไม่ได้ติดตั้ง กรุณาติดตั้งก่อนลงทะเบียน"
        );
        return;
      }
      ...
      if (isConnected && account) {
        showSuccess("เชื่อมต่อกระเป๋าเงินสำเร็จ โปรดรอสักครู่...");

        // Registration is just connecting wallet in this system
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">
          ลงทะเบียนใช้งาน VeriFact
        </h2>
        <p className="mt-2 text-center text-sm text-gray-600">
          เชื่อมต่อกระเป๋าเงินดิจิทัลของคุณเพื่อเริ่มใช้งาน
        </p>
      </div>
        ...
        <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
          ...
          <div>
            ...
            >
              ...
              {isConnected
                ? "ลงทะเบียนด้วย Wallet นี้"
                : "เชื่อมต่อ Wallet และลงทะเบียน"}
            </button>
          </div>
