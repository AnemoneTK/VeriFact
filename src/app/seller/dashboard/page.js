...
export default function SellerDashboard() {
  ...
  const handleTransferToCustomer = async (productId) => {
    const buyerAddress = prompt("กรอกที่อยู่กระเป๋าเงินของผู้ซื้อ (0x...):");
    if (!buyerAddress) return;

    const price = prompt("กรอกราคาที่ขาย (บาท):");
    if (!price) return;
                        ...
                        <div className="flex space-x-2">
                          ...
                          {product?.currentOwner?.toLowerCase() ===
                            ...
                            product.isActive && (
                              <button
                                onClick={() =>
                                  handleTransferToCustomer(product.productId)
                                }
