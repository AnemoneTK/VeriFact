...
export default function VerifyResultPage({ params }) {
  ...
  const { verifactContract, isConnected, connectWallet, account } = useWeb3();
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      ...
      try {
        // ถ้าผู้ใช้เชื่อมต่อกระเป๋าเงินแล้ว ใช้การเรียก contract โดยตรง
        if (isConnected && verifactContract) {
          ...
          if (isSerialNumber) {
            actualProductId = await verifactContract.methods
              .findProductBySerialNumber(productId)
              .call();
          ...
          // ดึงข้อมูลสินค้า
          const productData = await verifactContract.methods
            .getProduct(actualProductId)
            .call();

          // แปลงข้อมูลสินค้า
          const formattedProduct = {
            productId: productData[0]?.toString() || actualProductId,
          ...
          const history = await verifactContract.methods
            .getTransferHistory(actualProductId)
            .call();
          setTransferHistory(history);

          // ตั้งค่าความถูกต้องของสินค้า
          setIsAuthentic(true);
          setVerificationStatus("verified");
        }
        // ถ้าผู้ใช้ไม่ได้เชื่อมต่อกระเป๋าเงิน ใช้ API แทน
        else {
          // เรียกใช้ API ที่เซิร์ฟเวอร์
          const response = await fetch(`/api/verify-product?id=${productId}`);
          ...
          <div className="px-6 py-8">
            {isLoading ? (
              ...
              <>
                ...
                {/* ข้อมูลสินค้า */}
                {/* ข้อมูลสินค้า - แก้ไขส่วนที่แสดงรายละเอียด */}
                {productData && (
                  <div className="mb-8">
                    ...
                    <div className="bg-gray-50 rounded-lg p-4">
                      <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {/* แยกแสดงชื่อสินค้า รุ่น และหมายเลขซีเรียล */}
                        {productData.details &&
                          productData.details.split("|").length >= 3 && (
                            <>
                              <div>
                                <dt className="text-sm font-medium text-gray-500">
                                  ชื่อสินค้า
                                ...
                                <dd className="mt-1 text-sm text-gray-900">
                                  {productData.details.split("|")[0].trim()}
                                </dd>
                              </div>
                              <div>
                                <dt className="text-sm font-medium text-gray-500">
                                ...
                                <dd className="mt-1 text-sm text-gray-900">
                                  {productData.details.split("|")[1].trim()}
                                </dd>
                              </div>
                              <div className="bg-yellow-50 p-2 rounded-md border border-yellow-200 md:col-span-2">
                                <dt className="text-sm font-medium text-gray-700">
                                  หมายเลขซีเรียล
                                </dt>
                                <dd className="mt-1 text-sm font-mono text-gray-900 font-semibold">
                                  {productData.details.split("|")[2].trim()}
                                </dd>
                              </div>
                            </>
                          )}
